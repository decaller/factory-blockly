function createRobotSprite(scene, worldX, worldY, bodyColor) {
  const robot = scene.add.container(worldX, worldY);
  ...
  robot.add([body, eye]);
  robot.setSize(scene.tileSize - 10, scene.tileSize - 10);
...
export function drawGrid() {
  ...
  for (let y = 0; y < this.gridSize; y++) {
    this.add
      .text(-4, y * this.tileSize + this.tileSize / 2, `${y}`, labelStyle)
      .setOrigin(1, 0.5);
  }
}
...
export function drawConveyorBelts() {
  ...
  const dArrow = this.add
    .text(dArrowX, dArrowY, "▶", {
      fontSize: "28px",
      color: "#aaccff",
    })
    .setOrigin(0.5);
  ...
  const rArrow = this.add
    .text(rArrowX, rArrowY, "▶", {
      fontSize: "28px",
      color: "#a5d6a7",
    })
    .setOrigin(0.5);
...
export function createLevel() {
  ...
  const robot2GridX = 1;
  const robot2GridY = this.gridSize - 2;

  this.robot = createRobotSprite(this, robotX, robotY, 0x00ff00);
  this.robot2 = createRobotSprite(
    this,
    robot2GridX * this.tileSize + this.tileSize / 2,
    robot2GridY * this.tileSize + this.tileSize / 2,
    0x4fc3f7,
  );
  ...
  this.robot2.gridX = robot2GridX;
  this.robot2.gridY = robot2GridY;
  this.robot2.direction = 0;

  this.score = 0;
  this.scoreText = this.add
    .text(this.gridSize * this.tileSize - 8, 8, "Score: 0", {
      fontSize: "16px",
      color: "#ffffff",
      fontStyle: "bold",
    ...
    .setOrigin(1, 0)
    .setDepth(10);
  this.ticksAfterLastScoreText = this.add
    .text(this.gridSize * this.tileSize - 8, 36, "Ticks after last score: 0", {
      fontSize: "14px",
      color: "#ffffff",
      fontStyle: "bold",
      backgroundColor: "#00000088",
    ...
    .setOrigin(1, 0)
    .setDepth(10);
  this.avgTicksAfterLastScoreText = this.add
    .text(
      this.gridSize * this.tileSize - 8,
      64,
      "Average ticks after last score: 0.00",
      {
        fontSize: "14px",
        color: "#ffffff",
        fontStyle: "bold",
